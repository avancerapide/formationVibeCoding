...
Jeu du serpent de validation des connaissances, alimenté par une banque de questions SQLite.

## Lancer l’application

Aucune dépendance n’est nécessaire : Python 3 et son module `sqlite3` suffisent.

```bash
python3 server.py
```

Ouvrez ensuite <http://127.0.0.1:8000>. Au premier démarrage, le serveur crée `data/cybersnake.db` et l’initialise avec les questions et réponses versionnées dans `data/seed.sql`.
